...
async function main() {
  ...
  // Enable CORS for all origins (required for MCP aggregators)
  app.use(cors());
  ...
  app.get('/health', (_req: Request, res: Response) => {
    res.json({ status: 'ok', service: 'my-network-mcp', version: '2.0.0' });
  });
  ...
  app.post('/mcp', async (req: Request, res: Response) => {
    try {
      ...
      if (sessionId && transports[sessionId]) {
        ...
        // Store transport when session is established
        transport.onclose = () => {
        ...
        // We'll store after handling the request since sessionId is set during handling
        await transport.handleRequest(req, res, req.body);

        // Store the transport if a session was created
        ...
        // Invalid request - no session and not an initialization
        res.status(400).json({
          jsonrpc: '2.0',
          error: {
            code: -32600,
  ...
  app.listen(PORT, HOST, () => {
    console.log(`My Network MCP server running on http://${HOST}:${PORT}`);
    console.log(`MCP endpoint: http://${HOST}:${PORT}/mcp`);
    console.log(`Health check: http://${HOST}:${PORT}/health`);
    console.log(`Network map location: ${storage.getMapPath()}`);
  });
